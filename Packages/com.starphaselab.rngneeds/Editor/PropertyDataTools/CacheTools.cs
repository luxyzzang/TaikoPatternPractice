...
namespace RNGNeeds.Editor
{
    internal static class CacheTools
    {
        internal static void SetupProperties(this PropertyData propertyData, ProbabilityListDrawer plDrawer)
        {
            ...
            for (var i = 0; i < propertyData.p_ProbabilityItems.arraySize; i++)
            {
                ...
                if (i == 0)
                {
                    ...
                    if(newItemProperty.p_Value.propertyType == SerializedPropertyType.Integer || newItemProperty.p_Value.propertyType == SerializedPropertyType.Float)
                    {
                        propertyData.NumericPropertyFieldOffset = 4f;
                        propertyData.NumericPropertyLabelText = "|";
                    } 
                    else
                    {
                        propertyData.NumericPropertyFieldOffset = 0f;
                        propertyData.NumericPropertyLabelText = string.Empty;
                    }
                }
            }
        ...
        {
            var itemInfoCache = propertyData.ItemInfoCache[index];
            var baseProbability = propertyData.GetBaseProbability(index);
            if (baseProbability >= 1f)
            {
                itemInfoCache.stripePercentage = "100 %";
                itemInfoCache.listElementPercentage = "100 %";
            }
            else
            {
                itemInfoCache.stripePercentage = baseProbability.ToString($"P{propertyData.DrawerSettings.StripePercentageDigits.ToString()}", CultureInfo.InvariantCulture);
                itemInfoCache.listElementPercentage = baseProbability.ToString($"P{propertyData.DrawerSettings.ItemPercentageDigits.ToString()}", CultureInfo.InvariantCulture);    
            }
