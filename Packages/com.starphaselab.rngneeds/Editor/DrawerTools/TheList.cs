...
namespace RNGNeeds.Editor
{
    internal static class TheList
    {
        ...
        internal static void DrawElement(PropertyData propertyData, Rect rect, int index, FloatLabelField floatLabelField, IntLabelField intLabelField, float itemRectAlignmentFixX)
        {
            ...
            var indexRect = new Rect(toggleRect.xMax, rect.position.y + 1f, 24f, EditorGUIUtility.singleLineHeight);
            ...
            var probRect = new Rect(influenceProviderToggleRect.xMax + 4f, rect.position.y + 1f, 32f + propertyData.DrawerSettings.ItemPercentageDigits * 9f, EditorGUIUtility.singleLineHeight);
            ...
            {
                m_DepletableItemToggleRect.Set(colorRect.xMax + 4f, rect.position.y + 2f, 16f, EditorGUIUtility.singleLineHeight);
                ...
                m_MaxUnitsRect.Set(m_DepletableItemSeparatorRect.xMax + 2f, rect.position.y + 2f, 22f, EditorGUIUtility.singleLineHeight);
                lastX = m_MaxUnitsRect.xMax;
            }
            
            var propertyWidth = Mathf.Abs(removeButtonRect.xMin - lastX - 10f - itemRectAlignmentFixX);
            var itemRect = new Rect(lastX + 4f - propertyData.NumericPropertyFieldOffset + itemRectAlignmentFixX, rect.position.y + 2f, propertyWidth + propertyData.NumericPropertyFieldOffset, propertyData.ItemPropertyCache[index].p_Value.isExpanded ? rect.height - 3f : EditorGUIUtility.singleLineHeight);
            ...
            {
                m_TempContent.text = propertyData.NumericPropertyLabelText;
                EditorGUIUtility.labelWidth = propertyData.NumericPropertyFieldOffset;
            }
