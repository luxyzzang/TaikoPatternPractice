...
namespace RNGNeeds.Editor
{
    ...
    internal class ProbabilityListDrawer : LabDrawerBase
    {
        ...
        private Rect m_DisplayOptionsDropdownRect;
        ...
        private PropertyData AddPropertyData(SerializedProperty property)
        {
            ...
            if (newPropertyData.ValueIsGenericType) itemRectAlignmentFixX = 10f;
        ...
        protected override void BaseDraw(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            var shouldDrawIndividualToggleButtons = RNGNStaticData.Settings.DrawerDisplayOptionsButtons == DrawerDisplayOptionsButtons.IndividualButtons;
            
            var themeSectionButtonContent = PLDrawerUtils.GetThemeSectionButtonContent(RNGNStaticData.Settings.DrawerSectionButtons, cpd.DrawerSettings);
            var pickSectionButtonContent = PLDrawerUtils.GetPickSectionButtonContent(RNGNStaticData.Settings.DrawerSectionButtons, cpd);
            ...
            if (shouldDrawIndividualToggleButtons)
            {
                ...
                m_DimColorsButtonRect.Set(m_DisplayPercentageButtonRect.xMax + 6f, optionsYPosition, 20f, 20f);
                ...
                m_SeparatorBRect.Set(m_NormalizeBarsButtonRect.xMax, optionsYPosition, 14f, 20f);
            }
            else
            {
                m_DisplayOptionsDropdownRect.Set(m_SeparatorARect.xMax, optionsYPosition, 30f, 20f);
                m_SeparatorBRect.Set(m_DisplayOptionsDropdownRect.xMax, optionsYPosition, 14f, 20f);
            }
            
            m_ShowWeightsButtonRect.Set(m_SeparatorBRect.xMax, optionsYPosition, 20f, 20f);
            ...
            EditorGUI.BeginProperty(position, label, property);
            {
                ...
                if (GUI.Button(m_ShowListEntriesButtonRect, m_TempContent, cpd.DrawerSettings.HideListEntries ? PLDrawerTheme.HideEntriesButtonOn : PLDrawerTheme.HideEntriesButtonOff))
                {
                ...
                else
                {
                    ...
                    if (EditorGUI.DropdownButton(m_DisplayOptionsDropdownRect, PLDrawerContents.DisplayOptionsDropdown, FocusType.Passive, PLDrawerTheme.DisplayOptionsDropdownButton))
                    {
                        DropdownMenus.DisplayOptionsDropdownMenu(cpd);
                        cpd.DrawerDisplayOptionsDropdownMenu.DropDown(m_DisplayOptionsDropdownRect);
                    }
                ...
                {
                    if(GUI.Button(new Rect(m_PickSectionButtonRect.xMax + 6f, optionsYPosition, 30f, 20f), "M", PLDrawerTheme.CriticalActionButtonHover))
                    {
                ...
                switch (cpd.DrawerSettings.DrawerOptionSection)
                {
                    ...
                    case DrawerOptionSection.Picks:
                        ...
                        var pickCountCurveRect = new Rect(pickCountMaxRect.xMax + 4f, subOptionsYPosition + 1f, 32f, EditorGUIUtility.singleLineHeight);
                        var rollSectionSeparatorRect = new Rect(pickCountCurveRect.xMax, subOptionsYPosition, 14f, 20f);
                        
                        m_TestRollButtonRect.Set(rollSectionSeparatorRect.xMax, subOptionsYPosition, 48f, 20f);
                        ...
                        if(showAdvancedOptions)
                        {
                            ...
                            if (m_CurrentEvent.type == EventType.Repaint)
                            {
                                PLDrawerTheme.Separator.Draw(new Rect(cpd.SubOptionsRect.xMax - 190f, subOptionsSecondRowYPosition, 18f, 20f), false, false, false, false);
                            }
                            
                            if (GUI.Button(new Rect(cpd.SubOptionsRect.xMax - 208f, subOptionsSecondRowYPosition, 20f, 20f), cpd.p_WeightsPriority.boolValue ? PLDrawerContents.WeightsPriorityButtonOn : PLDrawerContents.WeightsPriorityButtonOff, cpd.p_WeightsPriority.boolValue ? PLDrawerTheme.WeightsPriorityButtonOn : PLDrawerTheme.WeightsPriorityButtonOff))
                            {
                            ...
                            if (GUI.Button(new Rect(cpd.SubOptionsRect.xMax - 232f, subOptionsSecondRowYPosition, 20f, 20f), PLDrawerContents.ResetWeightsButton, PLDrawerTheme.ResetWeightsButton))
                            {
                            ...
                            EditorGUI.BeginChangeCheck();
                            cpd.p_BaseWeight.intValue = EditorGUI.IntField(new Rect(cpd.SubOptionsRect.xMax - 286f, subOptionsSecondRowYPosition, 50f, 20f), GUIContent.none, cpd.p_BaseWeight.intValue, PLDrawerTheme.InputField);
                            if (EditorGUI.EndChangeCheck())
                            {
                            ...
                            if (GUI.Button(new Rect(cpd.SubOptionsRect.xMax - 82f, subOptionsSecondRowYPosition, 76f, 20f), PLDrawerContents.KeepSeedButton, cpd.p_KeepSeed.boolValue ? PLDrawerTheme.OptionLinkButtonOn : PLDrawerTheme.OptionLinkButtonOff))
                            {
                            ...
                            if (cpd.p_KeepSeed.boolValue)
                            {
                                ...
                                var fieldSeedValue = EditorGUI.LongField(new Rect(cpd.SubOptionsRect.xMax - 176f, subOptionsSecondRowYPosition, 90f, 20f), PLDrawerContents.SeedReadoutField, cpd.p_Seed.uintValue, PLDrawerTheme.InputField);
                                cpd.p_Seed.uintValue = Convert.ToUInt32(Math.Clamp(fieldSeedValue, 0, uint.MaxValue));
                                #else
                                cpd.p_Seed.longValue = EditorGUI.LongField(new Rect(cpd.SubOptionsRect.xMax - 176f, subOptionsSecondRowYPosition, 90f, 20f), PLDrawerContents.SeedReadoutField, cpd.p_Seed.longValue, PLDrawerTheme.InputField);
                                #endif
                            }
                            else
                            {
                                EditorGUI.LabelField(new Rect(cpd.SubOptionsRect.xMax - 176f, subOptionsSecondRowYPosition, 90f, 20f), ((uint)cpd.p_Seed.intValue).ToString(), PLDrawerTheme.InputField);
                            }
                ...
                if (GUI.Button(m_AddButtonRect, PLDrawerContents.AddItemButton, cpd.p_ProbabilityItems.arraySize == 0 ? PLDrawerTheme.AddButton : PLDrawerTheme.AddButtonHighlight))
                {
                    cpd.AddItem();
                    GUIUtility.ExitGUI();
                }
