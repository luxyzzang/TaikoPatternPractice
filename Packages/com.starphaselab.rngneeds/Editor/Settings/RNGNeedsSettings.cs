...
namespace RNGNeeds.Editor
{
    ...
    public class RNGNeedsSettings : ScriptableSingleton<RNGNeedsSettings>
    {
        ...
        public DrawerSectionButtons DrawerSectionButtons => m_DrawerSectionButtons;
        public DrawerDisplayOptionsButtons DrawerDisplayOptionsButtons => m_DrawerDisplayOptionsButtons;
        ...
        private string UpdateDrawerSettings()
        {
            ...
            return RNGNStaticData.CurrentVersion;
        }
        ...
        // Global Settings
        [Header("Options")] 
        [Tooltip("Advanced setting will show additional options in the drawer.")]
        [SerializeField] private DrawerOptionsLevel m_DrawerOptionsLevel = DrawerOptionsLevel.Basic;
        [FormerlySerializedAs("m_DrawerOptionButtons")]
        [Tooltip("Show Compact, Full, or Informative section buttons in drawer Header. Use Compact if you have limited inspector width.")]
        [SerializeField] private DrawerSectionButtons m_DrawerSectionButtons = DrawerSectionButtons.Full;
        [Tooltip("Show all display option buttons in the drawer header, or group them in a dropdown. Use Dropdown if you have limited inspector width.")]
        [SerializeField] private DrawerDisplayOptionsButtons m_DrawerDisplayOptionsButtons = DrawerDisplayOptionsButtons.IndividualButtons;
        ...
        private void SetGlobalDefaults()
        {
            ...
            m_DrawerSectionButtons = DrawerSectionButtons.Full;
            m_DrawerDisplayOptionsButtons = DrawerDisplayOptionsButtons.IndividualButtons;
            ...
            m_EditorLogLevel = LogMessageType.Info | LogMessageType.Hint;
        ...
        public static void ImportPalettes()
        {
            ...
            var confirmMethod = EditorUtility.DisplayDialogComplex(RNGNStaticData.ImportPalettesWindowTitle, RNGNStaticData.ImportPalettesWindowText, RNGNStaticData.ImportPalettesWindowConfirm, RNGNStaticData.ConfirmWindowCancel, RNGNStaticData.ImportPalettesWindowConfirmReplace);
            
            switch (confirmMethod)
            {
                case 0:     // Add New Palettes
                    break;
                case 1:     // Cancel
                    return;
                case 2:     // Replace Existing List
            ...
            if (loadedPalettes != null)
            {
                ...
                {
                    instance.colorPalettes = loadedPalettes.List;
                    RLogger.Log("Existing list of palettes was cleared.", LogMessageType.Info);
                    RLogger.Log($"Imported {loadedPalettes.List.Count} palettes from {userPath}.", LogMessageType.Info);
                    return;
                }
                
                var newPalettesCount = 0;
                
                foreach (var loadedPalette in loadedPalettes.List)
                {
                    var existingPalette = instance.colorPalettes.Find(p => p.palettePath.Equals(loadedPalette.palettePath));
                    if (existingPalette != null) continue;
                    instance.colorPalettes.Add(loadedPalette);
                    newPalettesCount++;
                    Debug.Log($"Adding new palette: {loadedPalette.palettePath}");

                }
                
                RLogger.Log($"{newPalettesCount} new color palettes imported from {userPath}", LogMessageType.Info);
            }
            else
            {
                RLogger.Log($"Color Palettes import failed! Could not load palettes from {userPath}.", LogMessageType.Warning);
            }
        }
