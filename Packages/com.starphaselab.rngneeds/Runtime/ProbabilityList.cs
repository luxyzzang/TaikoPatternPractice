...
namespace RNGNeeds
{
    ...
    /// </summary>
    [Serializable]
    public class ProbabilityList<T> : IProbabilityList, IEnumerable<ProbabilityItem<T>>
    {
        ...
        /// <summary>
        /// Creates new <see cref="ProbabilityItem{T}"/>s for the provided values and adds them to the list.
        /// Items are added one by one using <see cref="AddItem(T, bool, bool)"/>. If "locked" parameter is set to True, new items will be locked after all items are added and their probabilities are calculated.
        /// </summary>
        /// <param name="values">The values to be added.</param>
        /// <param name="enabled">Specifies whether the added items should be enabled in the list. The default is True.</param>
        /// <param name="locked">Specifies whether the added items should be locked in the list. The default is False.</param>
        public void AddItems(IEnumerable<T> values, bool enabled = true, bool locked = false)
        {
            if (locked)
            {
                var countBefore = ItemCount;
                foreach (var item in values) AddItem(item, enabled);
                for(var i = countBefore; i < ItemCount; i++) ProbabilityItems[i].Locked = true;
            }
            else foreach (var item in values) AddItem(item, enabled);
        }
        ...
        /// Items are added one by one using <see cref="AddItem(T, bool, bool)"/>. If "locked" parameter is set to True, new items will be locked after all items are added and their probabilities are calculated.
        /// </summary>
        /// <param name="enabled">Specifies whether the added items should be enabled in the list.</param>
        /// <param name="locked">Specifies whether the added items should be locked in the list.</param>
        /// <param name="values">The values to be added.</param>
        public void AddItems(bool enabled, bool locked, params T[] values)
        {
            AddItems(values.ToList(), enabled, locked);
        }
        ...
        public IEnumerator<ProbabilityItem<T>> GetEnumerator()
        {
            foreach (var item in ProbabilityItems)
            {
                yield return item;
